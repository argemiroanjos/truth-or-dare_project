    ...
    <main className="relative flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-indigo-950 to-black p-4 text-white overflow-hidden">
      ...
      <div className="text-center absolute top-10 z-20">
        <h1 className="text-2xl font-display">
          É a vez de <span className="font-bold text-cyan-400">{players[currentPlayerIndex].name}</span>
        </h1>
        <p className="text-gray-400">Clique na garrafa para girar!</p>
      </div>
      <div 
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 cursor-pointer group"
      >
        <svg 
          className="w-12 h-28 text-pink-500 group-hover:scale-110 transition-transform duration-300" 
          fill="none" 
          viewBox="0 0 48 112" 
      ...
      <div className="absolute bottom-10 flex gap-4 z-20">
        <button className="bg-cyan-500 text-white font-bold text-xl py-4 px-12 rounded-lg shadow-lg hover:bg-cyan-600">
          VERDADE
        </button>
        <button className="bg-pink-600 text-white font-bold text-xl py-4 px-12 rounded-lg shadow-lg hover:bg-pink-700">
          DESAFIO
        </button>
      </div>
